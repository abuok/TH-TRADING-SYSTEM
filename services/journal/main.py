...
def log_outcome(
    ...
    db_outcome = JournalTradeOutcome(
        setup_id=setup_id, is_win=is_win, r_multiple=r_multiple, pnl=pnl, notes=notes
    )
    db.add(db_outcome)
...
def log_ticket_transition(
    ...
    existing = (
        db.query(JournalTicketTransition)
        .filter(
    ...
    db_transition = JournalTicketTransition(
        ticket_id=ticket_id, transition_type=transition_type, details_json=details
    )
...
def daily_report(db: Session = Depends(db_session.get_db)):
    ...
    outcomes = (
        db.query(JournalTradeOutcome)
        .filter(JournalTradeOutcome.timestamp >= today)
        .all()
    )
    ...
    missed_aplus = sum(
        1 for s in setups if s.score_label == "A+" and s.status == "MISSED"
    )
    ...
    potential_profit_missed = sum(
        s.setup_score / 20.0
        for s in setups
        if s.score_label == "A+" and s.status == "MISSED"
    )
...
def weekly_report(db: Session = Depends(db_session.get_db)):
    ...
    outcomes = (
        db.query(JournalTradeOutcome)
        .filter(JournalTradeOutcome.timestamp >= one_week_ago)
        .all()
    )
