...
@app.command()
def scan(
    csv_path: str = "data/sample_ohlcv.csv", days: int = 30, timeframe: str = "1H"
):
    """
    ...
    console.print(
        f"[blue]Starting technical scan on {csv_path} for the last {days} days...[/blue]"
    )
    ...
    table.add_row(
        "Avg Close Price", f"{sum(c.close for c in candles) / len(candles):.2f}"
    )
