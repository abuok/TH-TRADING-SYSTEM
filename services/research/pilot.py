...
def fetch_session_metrics(db: Session, target_date: date) -> PilotSessionRecord:
    ...
    tickets = (
        ...
        .filter(OrderTicket.created_at >= start_dt, OrderTicket.created_at < end_dt)
        .all()
    )
    ...
    sorted_approved = sorted(
        [t for t in approved if t.executed_at], key=lambda x: x.executed_at
    )
    cumulative_r = 0.0
    peak_r = 0.0
    session_dd = 0.0
    ...
    # 3. Reliability
    stale_logs = (
        db.query(QuoteStaleLog)
        .filter(QuoteStaleLog.created_at >= start_dt, QuoteStaleLog.created_at < end_dt)
        .all()
    )
    max_stale = max([log.stale_duration_seconds for log in stale_logs] + [0.0])
    violations = [
        log for log in stale_logs if log.stale_duration_seconds > 30.0
    ]  # Threshold crossref
...
def evaluate_gate(
    ...
    if session.reliability_metrics.get("max_quote_stale_seconds", 0) > config.get(
        "max_quote_stale_seconds", 30.0
    ):
        fail_reasons.append(
            f"Quote Staleness Spike ({session.reliability_metrics.get('max_quote_stale_seconds'):.1f}s)"
        )

    if session.process_metrics.get("execution_prep_override_rate", 0) > config.get(
        "execution_prep_override_rate", 10.0
    ):
        fail_reasons.append("Too many manual overrides (rate)")

    if session.process_metrics.get("overrides", 0) > config.get(
        "max_overrides_per_session", 1
    ):
        fail_reasons.append(
            f"Overrides exceeded ({session.process_metrics.get('overrides')})"
        )

    if session.process_metrics.get("median_time_to_review_seconds", 0) > config.get(
        "max_median_time_to_review_seconds", 300
    ):
        fail_reasons.append("Operator response time too slow")

    if session.process_metrics.get("expired_ticket_rate", 0) > config.get(
        "expired_ticket_rate", 15.0
    ):
        fail_reasons.append("Too many expired tickets")

    if session.process_metrics.get("approved", 0) < config.get(
        "min_approved_trades", 0
    ):
        fail_reasons.append(
            f"Low trade volume ({session.process_metrics.get('approved')})"
        )

    if session.performance_metrics.get("approved_expectancy_r", 0) < config.get(
        "approved_expectancy_R", 0.05
    ):
        fail_reasons.append(
            f"Negative/Low Expectancy ({session.performance_metrics.get('approved_expectancy_r'):.2f}R)"
        )

    if session.performance_metrics.get("expectancy_delta_vs_baseline", 0) < config.get(
        "min_expectancy_delta_vs_baseline_R", 0.0
    ):
        fail_reasons.append(
            f"Expectancy Delta fail ({session.performance_metrics.get('expectancy_delta_vs_baseline'):.2f}R)"
        )

    if session.performance_metrics.get("win_rate_pct", 0) < config.get(
        "min_win_rate_pct", 40.0
    ):
        fail_reasons.append("Win rate too low")

    if session.performance_metrics.get("max_drawdown_r", 0) < config.get(
        "max_drawdown_R_per_session", -2.0
    ):
        fail_reasons.append(
            f"Session DD Breach ({session.performance_metrics.get('max_drawdown_r'):.2f}R)"
        )

    if "max_break_even_rate_pct" in config:
        if (
            session.performance_metrics.get("break_even_rate_pct", 0)
            > config["max_break_even_rate_pct"]
        ):
...
def generate_next_week_plan(db: Session, start_date: date, end_date: date) -> List[str]:
    ...
    recent_tuning = (
        ...
        .filter(
            TuningProposalLog.status == "OPEN", TuningProposalLog.created_at >= start_dt
        )
        .all()
    )
            ...
            try:
                for p in tune.data.get("proposals", []):
                    if p.get("status") != "REJECT":
                        plan.append(
                            f"Review Tuning Proposal #{p.get('id')}: {p.get('title')} ({p.get('target')})"
                        )
            except Exception:
                pass
...
def build_pilot_scorecard(
    ...
    while current_date <= end_date:
        ...
        # Save Log
        log = (
            db.query(PilotSessionLog)
            .filter(PilotSessionLog.session_id == session_rec.session_id)
            .first()
        )
        if not log:
            log = PilotSessionLog(
    ...
    overall_pass = (
        "PASS" if passes >= (total_days * 0.8) else "FAIL"
    )  # 80% passing days required
    ...
    sc_log = (
        ...
        .filter(PilotScorecardLog.scorecard_id == scorecard.scorecard_id)
        .first()
    )
    if not sc_log:
