...
def _log_hindsight_event(ticket_id: str, outcome_label: str, realized_r: float):
    ...
    try:
        httpx.post(
            ...
            json={"hindsight_outcome": outcome_label, "hindsight_realized_r": realized_r},
...
def walk_forward(ticket: OrderTicket, future_candles: List[Candle], max_candles: int = 1440) -> HindsightOutcome:
    """
...
def process_ticket_hindsight(db: Session, ticket_id: str, candles: List[Candle]) -> Optional[HindsightOutcome]:
...
def get_hindsight_summary(db: Session, target_date: str) -> dict:
    ...
    return {
        ...
        } if costliest and costliest.realized_r > 0 else None
    }
