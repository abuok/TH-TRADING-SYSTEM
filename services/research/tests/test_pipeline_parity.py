...
def _make_phx_trigger_candles(base: float = 2000.0):
    ...
    for i in range(3):
        candles.append(Candle(
            timestamp=_ts(i), open=base, high=base + i + 1, low=base - 1, close=base + i, volume=1000
        ))
    ...
    for i in range(3, 9):
        candles.append(Candle(
            timestamp=_ts(i), open=base + 2, high=base + 3, low=base + 1, close=base + 2, volume=1000
        ))
    # Sweep candle: low below min of last 9, closes above it
    min_low = min(c.low for c in candles[-9:])
    sweep_candle = Candle(
        timestamp=_ts(9), open=min_low + 2, high=min_low + 5, low=min_low - 1, close=min_low + 3, volume=2000
    )
    candles.append(sweep_candle)
    # SWEEP → DISPLACE: 2 of last 3 candles bullish
    candles.append(Candle(timestamp=_ts(10), open=base + 2, high=base + 6, low=base + 1, close=base + 5, volume=1000))
    candles.append(Candle(timestamp=_ts(11), open=base + 4, high=base + 8, low=base + 3, close=base + 7, volume=1000))
    candles.append(Candle(timestamp=_ts(12), open=base + 6, high=base + 10, low=base + 5, close=base + 9, volume=1000))
...
class TestPipelineParity:
    ...
    def test_mock_proxy_no_random_in_research_context(self):
        ...
        assert dxy == 0.00, "Research context must use deterministic (delta_pct=0) proxy data"
    ...
    def test_reproducibility_hash_changes_on_guardrails_version(self):
        ...
        r = ResearchRunResult(
            run_id="r1", pair="XAUUSD",
            start_date=_ts(0), end_date=_ts(10), timeframes=["H1"]
        )
...
class TestOutcomeEdgeCases:
    def _short_trade(self):
        return SimulatedTrade(
            ticket_id="sc_1", pair="GBPJPY", direction="SHORT",
            entry_price=190.0, stop_loss=192.0, take_profit_1=186.0,
        )
    ...
    def test_short_be_trigger(self):
        ...
        candles = [
            ...
            Candle(timestamp=_ts(0), open=190.0, high=190.5, low=188.0, close=189.0, volume=100),
            # Then SL hit at 192 — but SL was moved to entry (190)
            Candle(timestamp=_ts(1), open=189.0, high=191.0, low=188.5, close=190.5, volume=100),
        ]
    ...
    def test_long_tp2_hit(self):
        ...
        trade = SimulatedTrade(
            ticket_id="tp2_1", pair="XAUUSD", direction="LONG",
            entry_price=2000.0, stop_loss=1980.0,
            take_profit_1=2040.0, take_profit_2=2080.0
        )
        candles = [
            # TP1 hit — no exit because TP2 is set
            Candle(timestamp=_ts(0), open=2000, high=2045, low=1995, close=2040, volume=100),
            # TP2 hit
            Candle(timestamp=_ts(1), open=2040, high=2085, low=2035, close=2075, volume=100),
        ]
        result = simulate_outcome(trade.model_copy(), candles)
    ...
    def test_short_tp2_hit(self):
        ...
        trade = SimulatedTrade(
            ticket_id="stp2_1", pair="GBPJPY", direction="SHORT",
            entry_price=190.0, stop_loss=192.0,
            take_profit_1=186.0, take_profit_2=182.0
        )
        candles = [
            Candle(timestamp=_ts(0), open=190.0, high=190.5, low=185.0, close=186.5, volume=100),
            Candle(timestamp=_ts(1), open=186.5, high=187.0, low=181.5, close=183.0, volume=100),
        ]
        result = simulate_outcome(trade.model_copy(), candles)
        assert result.status == "WIN_TP2"
    ...
    def test_tiebreaker_sl_over_tp_long(self):
        ...
        trade = SimulatedTrade(
            ticket_id="tie_1", pair="XAUUSD", direction="LONG",
            entry_price=2000.0, stop_loss=1980.0, take_profit_1=2040.0
        )
        candles = [
            Candle(timestamp=_ts(0), open=2000, high=2100, low=1900, close=2050, volume=100)
        ]
        result = simulate_outcome(trade.model_copy(), candles)
    ...
    def test_tiebreaker_sl_over_tp_short(self):
        ...
        trade = SimulatedTrade(
            ticket_id="ties_1", pair="GBPJPY", direction="SHORT",
            entry_price=190.0, stop_loss=192.0, take_profit_1=186.0
        )
        candles = [
            Candle(timestamp=_ts(0), open=190.0, high=194.0, low=185.0, close=190.0, volume=100)
        ]
        result = simulate_outcome(trade.model_copy(), candles)
        assert result.status == "LOSS"
        assert result.realized_r == -1.0
    ...
    def test_zero_risk_distance_is_error(self):
        ...
        trade = SimulatedTrade(
            ticket_id="zero_1", pair="XAUUSD", direction="LONG",
            entry_price=2000.0, stop_loss=2000.0, take_profit_1=2040.0
        )
...
class TestCalibrationSanity:
    def _metrics(self, er=0.0, dd=0.0, win=50.0, n=100):
        return ResearchMetrics(
            total_trades=n, executed_trades=n, blocked_trades=0,
            win_rate_pct=win, expectancy_r=er, max_drawdown_r=dd
        )
    ...
    def test_calibration_report_sorts_high_before_medium(self):
        ...
        base_variant = ResearchVariant(name="baseline", config=baseline_cfg,
                                       metrics=self._metrics(er=0.10, dd=3.0, win=45.0, n=200))
        # High-confidence variant
        high_var = ResearchVariant(name="strict_score", config=var_cfg,
                                   metrics=self._metrics(er=0.20, dd=2.0, win=55.0, n=180))
        # Medium-confidence variant
        win_var = ResearchVariant(name="win_filter", config=CounterfactualConfig(min_setup_score=90.0),
        ...
        result = ResearchRunResult(
            ...
            timeframes=["H1"],
            variants={"baseline": base_variant, "strict_score": high_var, "win_filter": win_var}
        )
...
class TestRunReplayIntegration:
    ...
    def test_run_replay_uses_phx_not_mock(self):
        ...
        assert "_mock_detector" not in src, "simulator.py still references _mock_detector"
    ...
    def test_run_replay_produces_result_with_hash(self):
        ...
        for i in range(1, 51):
            candles.append(Candle(
                ...
                open=2010, high=2015, low=2005, close=2012, volume=500
            ))
