...
def analyze_variant(
    ...
    if win_rate_delta > 5.0 and er_delta >= -0.05 and retention < min_volume_retention and retention > 0.3:
        return Recommendation(
            id=recco_id,
...
def generate_calibration_report(
    ...
    for r in run_results:
        if baseline_name in r.variants:
            ...
            baseline_hash = str(hash(json.dumps(r.variants[baseline_name].config.model_dump(mode='json'), sort_keys=True)))
            break
            
    if not baseline_metrics:
        raise ValueError(f"Baseline variant '{baseline_name}' not found in any of the provided run results.")
        
    recommendations: List[Recommendation] = []
    evidence_tables: Dict[str, Any] = {
        "Metrics": {
            baseline_name: baseline_metrics.model_dump()
        }
    }
