...
def fetch_tuning_metrics(
    ...
    metrics = {
        ...
        "total_tickets": db.query(OrderTicket)
        ...
        "total_prep_logs": db.query(ExecutionPrepLog)
        .filter(ExecutionPrepLog.created_at.between(start_date, end_date))
        .count(),
        "expired_prep_logs": db.query(ExecutionPrepLog)
        .filter(
        ...
        .count(),
        # Trade Management
        "total_suggestions": db.query(ManagementSuggestionLog)
        .filter(ManagementSuggestionLog.created_at.between(start_date, end_date))
        .count(),
        "move_sl_suggestions": db.query(ManagementSuggestionLog)
        .filter(
        ...
        .count(),
        # Policy Router
        "risk_off_policies": db.query(PolicySelectionLog)
        .filter(
        ...
        "critical_alerts": db.query(ManagementSuggestionLog)
        .filter(
            ManagementSuggestionLog.created_at.between(start_date, end_date),
        ...
        if block_rate > 0.4:  # If more than 40% are blocked, suggest softening
            proposals.append(
                Proposal(
                    ...
                    evidence_refs=[
                        f"Block rate is unusually high at {block_rate * 100:.1f}% (> 40%)."
                    ],
                )
            )
        ...
        if expiry_rate > 0.3:  # If > 30% of prep tickets expire before execution
            proposals.append(
                Proposal(
                    ...
                    evidence_refs=[
                        f"Very high Prep Queue expiry rate: {expiry_rate * 100:.1f}%."
                    ],
                )
            )
    ...
    if metrics["move_sl_suggestions"] > 20:  # High volume of BE moves
        proposals.append(
            Proposal(
                ...
                evidence_refs=[
                    f"{metrics['move_sl_suggestions']} instances of SL to BE triggered. Volatility allows earlier risk reduction."
                ],
            )
        )
    ...
    if metrics["risk_off_policies"] > (total_tickets * 0.5):
        proposals.append(
            Proposal(
                ...
                evidence_refs=[
                    "RISK_OFF policy selected in > 50% of the recent window."
                ],
            )
        )
    ...
    if metrics["critical_alerts"] > 50:
        proposals.append(
            Proposal(
                ...
                evidence_refs=[
                    f"Over 50 CRITICAL alerts generated in the window ({metrics['critical_alerts']})."
                ],
            )
        )
...
def generate_tuning_report(db: Session, days_back: int = 7) -> TuningProposalReport:
    ...
    logger.info(
        f"Generated Tuning Proposal Report: {report_id} with {len(proposals)} proposals."
    )
