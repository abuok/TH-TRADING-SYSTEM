...
class DemoRunner:
    def __init__(self, dry_run: bool = True):
        ...
        self.orchestrator = Orchestrator(self.risk_config, dry_run=dry_run, notifier=self.notifier)
    ...
    async def run_e2e_flow(self):
        ...
        historical_candles = [
            Candle(timestamp=datetime.now(timezone.utc) - timedelta(hours=i), 
                   open=60000.0, high=61000.0, low=59000.0, close=60500.0, volume=1.0)
            for i in range(10)
        ]
        await self.orchestrator.pre_session_briefing(self.asset_pair, historical_candles)
        ...
        try:
            ...
            res = requests.post(f"{journal_url}/log/outcome?setup_id={setup_id}&is_win=True&r_multiple=3.0&pnl=150.0")
            if res.status_code == 200:
                logger.info("Successfully logged trade outcome to Journal Service.")
        except Exception as e:
            logger.warning(f"Could not reach Journal Service API ({e}). Skipping outcome log.")
        ...
        try:
            ...
            if report_res.status_code == 200:
                ...
                self.notifier.notify("E2E Demo Complete. Report Ready.", level="SUCCESS")
        except Exception as e:
