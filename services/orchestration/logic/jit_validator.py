...
class JITValidator:
    ...
    def validate(self, db: Session, ticket: OrderTicket) -> tuple[bool, str, str]:
        ...
        if ticket.expires_at and now_utc > ticket.expires_at.replace(
            tzinfo=timezone.utc
        ):
        ...
        alignment_decision = self.alignment_engine.evaluate(
            setup_data, pair_fund_db.data, ctx_db.data, db=db, now_nairobi=now_nairobi
        )

        if not alignment_decision.is_aligned:
            return (
                False,
                f"REJECTED_JIT: {'; '.join(alignment_decision.reason_codes)}",
                "",
            )
