...
async def test_orchestrator_notifications(risk_config, mock_notifier):
    ...
    orchestrator.get_latest_market_context = MagicMock(return_value=MarketContextPacket(
        schema_version="1.0.0", source="test", asset_pair="EURUSD", price=1.1, volume_24h=1.0
    ))
    ...
    setup = TechnicalSetupPacket(
        schema_version="1.0.0", asset_pair="EURUSD", strategy_name="PHX", 
        entry_price=1.1, stop_loss=1.09, take_profit=1.15, timeframe="1H"
    )
    ...
    approval = RiskApprovalPacket(
        schema_version="1.0.0", request_id="req1", status="BLOCK", 
        is_approved=False, risk_score=40.0, max_position_size=0.1, 
        rr_ratio=1.5, approver="RiskEngine", reasons=["Low RR"]
    )
