...
async def test_live_loop_dry_run(orchestrator):
    ...
    orchestrator.scan_for_setup = MagicMock(return_value=TechnicalSetupPacket(
        schema_version="1.0.0", asset_pair="BTCUSD", strategy_name="PHX", 
        entry_price=50000.0, stop_loss=49000.0, take_profit=53000.0, timeframe="1H"
    ))
...
def test_generate_decision(orchestrator):
    setup = TechnicalSetupPacket(
        schema_version="1.0.0", asset_pair="BTCUSD", strategy_name="PHX", 
        entry_price=50000.0, stop_loss=49500.0, take_profit=52000.0, timeframe="1H"
    )
    approval = RiskApprovalPacket(
        schema_version="1.0.0", request_id="req1", status="ALLOW", 
        is_approved=True, risk_score=80.0, max_position_size=0.1, 
        rr_ratio=4.0, approver="RiskEngine", reasons=[]
    )
