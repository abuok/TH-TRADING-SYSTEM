...
def test_xauusd_bearish_on_rising_dxy_and_yields():
    proxies = {
        "DXY": ProxySnapshot(symbol="DXY", current_value=105.0, previous_value=104.0, delta_pct=0.96),
        "US10Y": ProxySnapshot(symbol="US10Y", current_value=4.5, previous_value=4.1, delta_pct=1.2)
    }
...
def test_xauusd_bullish_on_risk_off():
    proxies = {
        "SPX": ProxySnapshot(symbol="SPX", current_value=4800.0, previous_value=5000.0, delta_pct=-4.0)
    }
    
    packet = evaluate_xauusd(proxies, [], datetime.now())
    
    assert packet.bias_label == "NEUTRAL" or packet.bias_label == "BULLISH" # Only +1 from risk-off, so might be neutral but score is positive
    assert packet.bias_score == 1.0

def test_gbpjpy_bullish_on_risk_on():
    proxies = {
        "SPX": ProxySnapshot(symbol="SPX", current_value=5100.0, previous_value=5000.0, delta_pct=2.0)
    }
...
def test_gbpjpy_bearish_on_boj_hawkishness():
    ...
    proxies = {
        "SPX": ProxySnapshot(symbol="SPX", current_value=5000.0, previous_value=5000.0, delta_pct=0.0)
    }
...
def test_evaluate_fundamentals_full_context():
    ...
    ctx = {
        "proxies": {
            ...
            "SPX": {"symbol": "SPX", "current_value": 4900.0, "delta_pct": -2.0} # Risk Off
        },
        "high_impact_events": [
            {"event": "BoE Interest Rate Hike Decision"}
        ]
    }
