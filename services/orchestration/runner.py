...
class Orchestrator:
    ...
    async def live_loop(self, asset_pair: str, interval_seconds: int = 60):
        ...
        while self.is_active:
            ...
            if self.governance.is_halted("HALT_ALL") or self.governance.is_halted(
                "HALT_SERVICE", "Orchestrator"
            ):
            ...
            # 3. Staleness Guard
            if not self.governance.validate_packet_freshness(
                "MarketContextPacket", context.timestamp, self.ttl_map
            ):
                logger.error(
                    f"Abandoning loop iteration for {asset_pair} due to stale context."
                )
                if self.dry_run:
                    break
                await asyncio.sleep(interval_seconds)
            ...
            if 50 <= score < 100:
                ...
                if (
                    last_score is None or abs(score - last_score) >= 10
                ):  # Only notify if significantly changed
                    self.notifier.notify(
                        f"Setup Forming on {asset_pair}: Score {score}", level="INFO"
                    )
            ...
            if setup:
                ...
                if not self.governance.validate_packet_freshness(
                    "TechnicalSetupPacket", setup.timestamp, self.ttl_map
                ):
                    if self.dry_run:
                        break
                    await asyncio.sleep(interval_seconds)
                ...
                self.notifier.notify(
                    f"Setup Execute-Ready on {asset_pair}", level="SUCCESS"
                )
                ...
                if not approval.is_approved:
                    self.notifier.notify(
                        f"Risk BLOCK on {asset_pair}: {', '.join(approval.reasons)}",
                        level="ERROR",
                    )
    ...
    def output_decision(self, decision: DecisionPacket):
        ...
        logger.info(
            f"Asset: {decision.asset_pair} | Strategy: {decision.strategy_name}"
        )
    ...
    def scan_for_setup(
        self, asset_pair: str, context: MarketContextPacket
    ...
    def generate_decision(
        self, setup: TechnicalSetupPacket, approval: RiskApprovalPacket, dry_run: bool
    ) -> DecisionPacket:
