...
async def risk_worker():
    ...
    while True:
        try:
            ...
            context_msgs = event_bus.consume("market_context", "risk_group", consumer_id, count=10)
            ...
            # Consume setups
            setup_msgs = event_bus.consume("technical_setups", "risk_group", consumer_id, count=5)
            for _, msgs in setup_msgs:
                for msg_id, payload in msgs:
                    ...
                    context_packet = MarketContextPacket(**current_context) if current_context else None
                    
                    if not context_packet:
                        logger.warning("Risk Service: No market context available. Blocking setup.")
                        ...
                        approval = risk_engine.evaluate(setup, context_packet, account_state)
                    
                    # Publish result
                    event_bus.publish("risk_approvals", approval.dict())
                    logger.info(f"Risk Service: Processed setup {setup.asset_pair}. Status: {approval.status}")
