...
def _log_transition(ticket_id: str, transition_type: str, details: dict):
    ...
    try:
        ...
        logger.warning(f"Failed to log transition {transition_type} for {ticket_id}: {e}")
...
def approve_ticket(db: Session, ticket_id: str) -> OrderTicket:
    ...
    if ticket.status != "IN_REVIEW":
        raise ValueError(f"Ticket must be IN_REVIEW to approve. Currently: {ticket.status}")
    ...
    _log_transition(ticket.ticket_id, "APPROVED", {"reviewed_at": ticket.reviewed_at.isoformat()})
    return ticket

def skip_ticket(db: Session, ticket_id: str, reason: SkipReasonEnum, notes: Optional[str] = None) -> OrderTicket:
    ...
    if ticket.status != "IN_REVIEW":
        raise ValueError(f"Ticket must be IN_REVIEW to skip. Currently: {ticket.status}")
    ...
    _log_transition(ticket.ticket_id, "SKIPPED", {
...
def close_ticket(
    ...
    if ticket.status != "APPROVED":
        raise ValueError(f"Ticket must be APPROVED to close. Currently: {ticket.status}")
    ...
    _log_transition(ticket.ticket_id, "CLOSED", {
...
def auto_expire_tickets(db: Session) -> int:
    ...
    expired_tickets = db.query(OrderTicket).filter(
        OrderTicket.status == "IN_REVIEW",
        OrderTicket.expires_at <= now
    ).all()
