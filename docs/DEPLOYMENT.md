...
This guide describes how to deploy the PHX Trading System to a fresh Ubuntu VPS for continuous operation.
...
- **Cleanups**: The backup script automatically rotates files based on `ARTIFACT_RETENTION_DAYS`.

---
*Note: Ensure your VPS firewall (UFW) allows incoming traffic on port 80 (Public UI) but restricts port 5432 (DB) and 6379 (Redis) to localhost only.*
