...
def test_full_pipeline_acceptance(risk_config):
    ...
    levels = asyncio.run(
        orchestrator.pre_session_briefing(asset_pair, historical_candles)
    )
    # Check that SOME levels were computed (could be Asia or London depending on time)
    assert (
        len(levels) > 0 or levels == {}
    )  # In memory sometimes returns {} if range missed, but 24h should hit
...
def test_timezone_validation():
    ...
    print(
        f"[TZ] Validated: {now_nairobi.tzname()} is {now_nairobi.utcoffset()} offset."
    )
