...
replacements = {
    ...
    # Conditional badges
    r'''style="background: \{\% if item\.severity == 'CRITICAL' \%\}rgba\(235, 47, 150, 0\.1\)\{\% elif item\.severity == 'WARNING' \%\}rgba\(242, 201, 76, 0\.1\)\{\% else \%\}rgba\(86, 204, 242, 0\.1\)\{\% endif \%\}; color: \{\% if item\.severity == 'CRITICAL' \%\}var\(--accent-pink\)\{\% elif item\.severity == 'WARNING' \%\}var\(--accent-yellow\)\{\% else \%\}var\(--accent-cyan\)\{\% endif \%\}; border: 1px solid \{\% if item\.severity == 'CRITICAL' \%\}var\(--accent-pink\)\{\% elif item\.severity == 'WARNING' \%\}var\(--accent-yellow\)\{\% else \%\}var\(--accent-cyan\)\{\% endif \%\}; font-size: 0\.55rem; font-weight: 900;"''': '''class="badge-sm {% if item.severity == 'CRITICAL' %}badge-pink{% elif item.severity == 'WARNING' %}badge-yellow{% else %}badge-cyan{% endif %}"''',
    r'''style="background: \{\% if item\.status == 'OPEN' \%\}rgba\(242, 201, 76, 0\.1\)\{\% else \%\}rgba\(39, 174, 96, 0\.1\)\{\% endif \%\}; color: \{\% if item\.status == 'OPEN' \%\}var\(--accent-yellow\)\{\% else \%\}var\(--accent-green\)\{\% endif \%\}; border: 1px solid \{\% if item\.status == 'OPEN' \%\}var\(--accent-yellow\)\{\% else \%\}var\(--accent-green\)\{\% endif \%\}; font-size: 0\.55rem; font-weight: 900;"''': '''class="badge-sm {% if item.status == 'OPEN' %}badge-yellow{% else %}badge-green{% endif %}"''',
    # Alignments
