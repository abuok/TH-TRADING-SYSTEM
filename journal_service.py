...
class JournalingService:
    ...
    def log_entry(cls, signal: SignalPacket, score: ScorePacket, risk: RiskPacket):
        # Determine status
        final_status = (
            "REJECTED" if risk.decision == "BLOCK" else "ACCEPTED (SIMULATED)"
        )
        ...
        print(
            f"[{entry.timestamp}] {signal.symbol} {signal.direction} -> {score.confidence_score}% -> {final_status} ({risk.reason})"
        )
