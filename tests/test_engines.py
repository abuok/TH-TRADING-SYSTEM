...
def test_lockout_trigger_blocks_execution():
    engine = LockoutEngine(config={
        "max_daily_loss_pct": 2.0,
        "max_consecutive_losses": 3
    })
    
    # Passing state
    state, msg = engine.evaluate({"daily_loss": 0.0, "account_balance": 10000.0, "consecutive_losses": 0}, db=None)
    ...
    # Under limit
    state, msg = engine.evaluate({"daily_loss": 50.0, "account_balance": 10000.0, "consecutive_losses": 0}, db=MockDB())
    assert state == LockoutState.TRADEABLE
    
    # Over daily loss limit (200 >= 2% of 10000)
    state, msg = engine.evaluate({"daily_loss": 200.0, "account_balance": 10000.0, "consecutive_losses": 0}, db=MockDB())
