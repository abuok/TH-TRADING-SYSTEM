...
from shared.logic.guardrails import GuardrailsEngine, _rule_quote_staleness
...
def test_drill_redis_disconnect_graceful_degradation(memory_db):
    ...
    with patch.object(bus.client, 'xadd', side_effect=redis.exceptions.ConnectionError("Connection refused")):
        ...
        with patch("shared.database.session.SessionLocal", return_value=memory_db):
            ...
            incident = memory_db.query(IncidentLog).filter_by(component="EventBus").first()
