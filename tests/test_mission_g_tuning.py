...
def test_tuning_metrics_gathering(db):
    ...
    # Seed 3 tickets
    t1 = OrderTicket(ticket_id="T1", setup_packet_id=1, risk_packet_id=1, pair="EURUSD", direction="BUY", entry_price=1.0, stop_loss=0.9, take_profit_1=1.2, lot_size=1.0, risk_usd=100.0, risk_pct=1.0, rr_tp1=2.0, idempotency_key="K1", created_at=past)
    t2 = OrderTicket(ticket_id="T2", setup_packet_id=2, risk_packet_id=2, pair="GBPUSD", direction="SELL", entry_price=1.0, stop_loss=0.9, take_profit_1=1.2, lot_size=1.0, risk_usd=100.0, risk_pct=1.0, rr_tp1=2.0, idempotency_key="K2", created_at=past)
    db.add_all([t1, t2])
    
    # 2 Guardrails Blocks
    g1 = GuardrailsLog(pair="EURUSD", discipline_score=50, hard_block=True, result_json={}, created_at=past)
    g2 = GuardrailsLog(pair="GBPUSD", discipline_score=40, hard_block=True, result_json={}, created_at=past)
    db.add_all([g1, g2])
...
def test_full_report_generation_integration(db):
    ...
    db.add(GuardrailsLog(pair="EURUSD", discipline_score=10, hard_block=True, result_json={}, created_at=past))
    db.add(GuardrailsLog(pair="GBPUSD", discipline_score=20, hard_block=True, result_json={}, created_at=past))
    
    # 2. High Execution Expire rate
    db.add(ExecutionPrepLog(ticket_id="T1", expires_at=now_eat, data={}, status="EXPIRED", created_at=past))
    ...
    # Check DB Logging
    log = db.query(TuningProposalLog).filter(TuningProposalLog.report_id == report.report_id).first()
