...
def test_pilot_metrics_aggregation(db):
    ...
    # Passing ticket
    t1 = create_mock_ticket(
        "PILOT-T1", "EURUSD", "APPROVED", dt_today + timedelta(hours=10), realized_r=1.5
    )
    db.add(t1)

    # Failed ticket (Expired)
    t2 = create_mock_ticket(
        "PILOT-T2", "GBPUSD", "EXPIRED", dt_today + timedelta(hours=11)
    )
    db.add(t2)
...
def test_full_scorecard_generation(db):
        ...
        for j in range(8):
            db.add(
                create_mock_ticket(
                    ...
                    "EURUSD",
                    "APPROVED",
                    dt_curr + timedelta(hours=10 + j),
                    realized_r=1.0,
                    hindsight_r=1.0,
                    executed_at=dt_curr + timedelta(hours=10 + j),
                )
            )
    ...
    assert (
        scorecard.pass_fail_summary == "PASS"
    )  # Win rate and expectancy should be good
    ...
    for i in range(5):
        ...
        db.add(
            ExecutionPrepLog(
                ...
                status="EXPIRED",
                created_at=dt_day,
                expires_at=dt_day + timedelta(hours=1),
                data={},
            )
        )
...
def test_gate_operator_response_failure(db):
    ...
    t = create_mock_ticket(
        "LATE-1",
        "EURUSD",
        "APPROVED",
        dt_day,
        reviewed_at=dt_day + timedelta(seconds=600),
    )
    db.add(t)
    db.commit()
...
def test_gate_session_drawdown_failure(db):
    ...
    db.add(
        create_mock_ticket(
            ...
            "APPROVED",
            dt_day,
            executed_at=dt_day + timedelta(hours=8),
            realized_r=-1.0,
        )
    )
    db.add(
        create_mock_ticket(
            ...
            "APPROVED",
            dt_day,
            executed_at=dt_day + timedelta(hours=9),
            realized_r=-1.5,
        )
    )
    db.commit()
