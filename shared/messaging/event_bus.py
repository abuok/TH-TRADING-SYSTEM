    ...
    def publish(self, stream_name: str, data: dict, retries: int = 3):
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                logger.warning(f"Redis publish failed to '{stream_name}' (attempt {attempt}/{retries}): {e}")
                if attempt == retries:
                    ...
                    try:
                        ...
                        logger.error(f"EventBus: Failed to write IncidentLog for Redis failure: {db_err}")
                    return None  # Fail gracefully
                time.sleep(0.5 * (2 ** (attempt - 1)))  # Exponential backoff (0.5s, 1.0s, 2.0s)
