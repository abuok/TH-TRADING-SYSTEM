...
def _build_system_status(db: Session) -> SystemStatus:
    ...
    last_incident = db.query(IncidentLog).order_by(
        IncidentLog.created_at.desc()
    ).first()
...
def _build_pair_overview(pair: str, db: Session) -> PairOverview:
    ...
    ctx_packet = db.query(Packet).filter(
        Packet.packet_type == "MarketContextPacket",
        Packet.data["asset_pair"].as_string() == pair
    ).order_by(Packet.created_at.desc()).first()
    if ctx_packet:
        metrics = ctx_packet.data.get("metrics", {})
        for k in ("asia_high", "asia_low", "london_high", "london_low", "prior_day_high", "prior_day_low", "london_open"):
            if k in metrics:
                key_levels[k] = metrics[k]

    # ── Setups ────────────────────────────────────────────────────────
    setup_packets = db.query(Packet).filter(
        Packet.packet_type == "TechnicalSetupPacket",
        Packet.data["asset_pair"].as_string() == pair
    ).order_by(Packet.created_at.desc()).limit(20).all()
    ...
    for sp in setup_packets:
        ...
        if len(top_setups) < 3:
            top_setups.append(SetupSummary(
                stage=stage,
    ...
    if not setup_packets:
        warnings.append(StaleWarning(field="setups", reason="No TechnicalSetupPacket found"))
...
def _build_operator_actions(
    ...
    for p in pairs:
        if p.has_stale_data:
            actions.append(OperatorAction(
                priority="MEDIUM", category="MONITOR",
        ...
        if p.latest_ticket and p.latest_ticket.status == "PENDING":
            actions.append(OperatorAction(
                priority="MEDIUM", category="EXECUTE",
...
def _build_delta(db: Session, now_nairobi: datetime, session_label: str) -> Optional[DeltaSection]:
    """Compare with the most recent previous briefing for the same session."""
    today = now_nairobi.date()
    prev = db.query(SessionBriefing).filter(
        SessionBriefing.session_label == session_label,
        SessionBriefing.date == today,
    ).order_by(SessionBriefing.created_at.desc()).first()
    ...
    # Current tickets
    cur_tickets: set = set()
    all_tickets = db.query(OrderTicket).order_by(
        OrderTicket.created_at.desc()
    ).limit(20).all()
    ...
    new_incidents = db.query(IncidentLog).filter(
        IncidentLog.created_at > prev_ts
    ).count()
    ...
    summary = f"Delta since {_age_str(prev.created_at)}: " + (", ".join(summary_parts) or "no significant changes")
...
def assemble_briefing(
    ...
    if system.active_kill_switches:
        global_warnings.append(f"⚠ KILL SWITCHES ACTIVE: {', '.join(system.active_kill_switches)}")
    if market.is_stale:
        global_warnings.append("⚠ Market context data is STALE — calendar inputs unreliable")
    for p in pairs:
        if p.has_stale_data:
...
def render_briefing_html(pack: BriefingPack) -> str:
    ...
    # ── Header
    type_badge = badge("INTRADAY DELTA", "#7c3aed") if pack.is_delta else badge("PRE-SESSION", "#0ea5e9")
    session_color = {"LONDON": "#10b981", "NEW YORK": "#f59e0b", "ASIA": "#6366f1"}.get(pack.session_label, "#64748b")

    warnings_html = ""
    if pack.global_warnings:
        items = "\n".join(f'<li style="color:#ef4444;margin:0.25rem 0;">{w}</li>' for w in pack.global_warnings)
        warnings_html = f"""
    ...
    ks_html = (
        " ".join(badge(k, "#ef4444") for k in ks) if ks
    ...
    inc_col  = {"CRITICAL": "#ef4444", "ERROR": "#f97316", "WARNING": "#f59e0b"}.get(inc_sev, "#94a3b8")
    sys_html = f"""
    ...
    ntw_html = f"<ul style='margin:0;padding-left:1.2rem;'>{ntw_items}</ul>" if ntw_items else '<span style="color:#10b981;">No restricted windows</span>'

    stale_badge = badge("STALE", "#ef4444") if pack.market_context.is_stale else badge("FRESH", "#10b981")
    mkt_html = f"""
    ...
    for po in pack.pair_overviews:
        bias_col = {"BULLISH": "#10b981", "BEARISH": "#ef4444", "NEUTRAL": "#f59e0b"}.get(po.bias, "#64748b")
        ticket_html = ""
        if po.latest_ticket:
            t = po.latest_ticket
            t_col = {"BLOCKED": "#ef4444", "PENDING": "#f59e0b", "TAKEN": "#10b981"}.get(t.status, "#94a3b8")
            ...
            ticket_html = "<p style='color:#64748b;margin:0.5rem 0;'>No ticket generated</p>"

        levels_html = ""
        if po.key_levels:
            levels_html = "<div style='display:flex;flex-wrap:wrap;gap:0.5rem;margin:0.5rem 0;'>"
        ...
        if po.stale_warnings:
            warn_items = "".join(f"<li style='color:#f97316;'>{w.field}: {w.reason}</li>" for w in po.stale_warnings)
