...
def evaluate_xauusd(proxies: Dict[str, ProxySnapshot], events: List[Dict], created_at: datetime) -> PairFundamentalsPacket:
    score = 0.0
    drivers = []
    ...
    if dxy and dxy.delta_pct is not None:
        if dxy.delta_pct > 0.1:
            score -= 2.0
            drivers.append(BulletItem(category="USD", text=f"DXY Rising (+{dxy.delta_pct:.2f}%)", impact=-1))
        elif dxy.delta_pct < -0.1:
            score += 2.0
            drivers.append(BulletItem(category="USD", text=f"DXY Falling ({dxy.delta_pct:.2f}%)", impact=1))
        else:
            drivers.append(BulletItem(category="USD", text=f"DXY Flat ({dxy.delta_pct:.2f}%)", impact=0))
            
    # 2. US10Y (US 10-Year Yields)
    us10y = proxies.get("US10Y")
    if us10y and us10y.delta_pct is not None:
        if us10y.delta_pct > 1.0: # Yields move differently, using 1% delta for significance
            score -= 2.0
            drivers.append(BulletItem(category="YIELDS", text=f"US10Y Rising (+{us10y.delta_pct:.2f}%)", impact=-1))
        elif us10y.delta_pct < -1.0:
            score += 2.0
            drivers.append(BulletItem(category="YIELDS", text=f"US10Y Falling ({us10y.delta_pct:.2f}%)", impact=1))
        else:
            drivers.append(BulletItem(category="YIELDS", text=f"US10Y Flat ({us10y.delta_pct:.2f}%)", impact=0))
    ...
    if spx and spx.delta_pct is not None:
        if spx.delta_pct < -0.5:
            ...
            drivers.append(BulletItem(category="RISK_SENTIMENT", text=f"Risk-On: SPX rally (+{spx.delta_pct:.2f}%)", impact=-1))
...
def evaluate_gbpjpy(proxies: Dict[str, ProxySnapshot], events: List[Dict], created_at: datetime) -> PairFundamentalsPacket:
    score = 0.0
    drivers = []
    ...
    if boj_hawkish:
        score -= 2.0
        drivers.append(BulletItem(category="CENTRAL_BANKS", text="BoJ Policy Event (JPY Strength Risk)", impact=-1))
    if boe_hawkish:
        score += 2.0
        drivers.append(BulletItem(category="CENTRAL_BANKS", text="BoE Policy Event (GBP Strength Risk)", impact=1))
...
def evaluate_fundamentals(context_packet_data: dict, created_at: datetime) -> Tuple[MarketMoversPacket, List[PairFundamentalsPacket]]:
    """
