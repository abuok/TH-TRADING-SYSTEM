...
def test_session_logic():
    ...
    # Mock time test
    mock_eat = SessionManager.TIMEZONE.localize(
        datetime(2024, 1, 1, 14, 0)
    )  # 2:00 PM EAT
    print(
        f"Mock 14:00 EAT -> London? {SessionManager.SESSIONS['LONDON']['start'] <= mock_eat.time() <= SessionManager.SESSIONS['LONDON']['end']}"
    )
